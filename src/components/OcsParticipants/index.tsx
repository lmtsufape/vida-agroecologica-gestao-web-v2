...
type OcsParticipantsHomeProps = {
  id: string;
};

export default function OCSParticipants({ id }: OcsParticipantsHomeProps) {
    ...
    if (!token) {
      ...
      getAllUsers(token)
        .then((response) => {
          ...
          const onlyAgricultores = allUsers.filter((user: any) =>
        ...
        const handleDesvincular = () => {
          if (token) {
            desvincularAgricultor(token, value)
              .then(() => {
                getUsersByOCS(token, id).then((response) =>
  ...
  const handleVincularAgricultor = () => {
    ...
    if (!token) {
      ...
      vincularAgricultorOrganizacao(token, selectedUser, id)
        .then(() => {
          getUsersByOCS(token, id).then((response) =>
            setParticipants(response.users),
          );
