...
const ReuniaoEditHome = ({ id }: ReuniaoEditHomeProps) => {
  ...
  const handleEditRegister = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await editReuniao(requestData, token, id);
      router.back();
    } catch (error) {
      console.dir(error, { depth: null, colors: true });
      if (error instanceof AxiosError) {
        console.log(
          `[editReuniao] ${JSON.stringify(error?.response?.data?.message)}`,
        );
...
export default ReuniaoEditHome;
